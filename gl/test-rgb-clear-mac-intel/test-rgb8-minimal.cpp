...
int main() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, 2, 2, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
  ...
  const bool INCUR_DRIVER_BUG = false;
...
void main() {
  o_FragColor = texture(u_sampler, vec2(0));
}
)";
...

  {
    ...
    glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &p);
    printf("%08x\n", p);
    assert((p & 0xff000000) == 0xff000000);
  }
